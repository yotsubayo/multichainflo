...
var _ = Describe("Gas", func() {
	Context("when estimating flo network fee", func() {
		It("should work", func() {
			ctx, cancel := context.WithCancel(context.Background())
			defer cancel()

			client := flo.NewClient(flo.DefaultClientOptions())

			// estimate fee to include tx within 1 block.
			fallback1 := uint64(123)
			gasEstimator1 := flo.NewGasEstimator(client, 1, pack.NewU256FromUint64(fallback1))
			...
			gasEstimator2 := flo.NewGasEstimator(client, 10, pack.NewU256FromUint64(fallback2))
			...
			gasEstimator3 := flo.NewGasEstimator(client, 100, pack.NewU256FromUint64(fallback3))

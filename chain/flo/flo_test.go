	...
	Context("when submitting transactions", func() {
		Context("when sending Flo to multiple addresses", func() {
			It("should work", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				pkhAddrUncompressed, err := floutil.NewAddressPubKeyHash(floutil.Hash160(wif.PrivKey.PubKey().SerializeUncompressed()), &flo.RegressionNetParams)
				Expect(err).ToNot(HaveOccurred())
				...
				tx, err := flo.NewTxBuilder(&flo.RegressionNetParams).BuildTx(inputs, recipients)
				Expect(err).ToNot(HaveOccurred())
				...
				// Submit the transaction to the Flo node. Again, this
